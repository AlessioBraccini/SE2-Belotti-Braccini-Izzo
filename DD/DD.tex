...
\begin{center}
    \begin{figure}[H]
        \includegraphics[scale=0.55, center]{assets/application-server-component.png}
...
\begin{itemize}
    ...
    It also manages the external APIs such as the Google Maps API and the OpenWeatherAPI.
...
\begin{center}
    \begin{figure}[H]
        \includegraphics[scale=0.6, center]{assets/SequenceDiagram/SignUp.png}
        \caption{Sign Up}
        \label{fig:signup}
    \end{figure}
\end{center}
...
If the user is a farmer, in a subsequent page, he/she chooses also the crop type the farm deals with.
...
\subsubsection{Farmers location} In order to locate a farm the user has two options:
    \begin{itemize}
        ...
        \item GPS coordinates: if the address hasn't been inserted by the user or, if the \textit{LocationModule} doesn't find the address, the user can manually specify the coordinates with the help of the \textit{MapService}.
    \end{itemize}

\subsubsection{Agronomist daily plan} An agronomist has to have a ready daily plan.
During the daytime it can be manually updated until the end of the day where the system automatically confirms the plan.
An empty plan means that the agronomist hasn't visited any farmer.
...
\begin{figure}[H]
    \centering
    \subfloat[\centering MapServiceManager]{{\includegraphics[scale=0.56]{assets/Test/farmLocationTest.png} }}%
    \qquad\qquad
    \subfloat[\centering WeatherManager]{{\includegraphics[scale=0.39]{assets/Test/weatherTest.png} }}%
...
\begin{figure}[H]
    \centering
    \subfloat[\centering ForumManager]{{\includegraphics[scale=0.5]{assets/Test/forumTest.png}}}%
    \qquad\qquad
    \subfloat[\centering LocationModule]{{\includegraphics[scale=0.5]{assets/Test/locationModuleTest.png}}}%
