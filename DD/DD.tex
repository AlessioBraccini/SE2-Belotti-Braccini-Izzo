...
The data used to calculate the score is all the one collected in the 12 months prior the computation. The score is worked out every time that $\eta$ of the farmer mutate (as a result of a new production entry), and the ranking is directly proportional to it. If farmers obtain the same score, they are going to be ranked in the same position as well.
