...
It also manages all the external APIs including Google Maps API and OpenWeatherAPI.
...
It also manages the news, the water irrigation service and the humidity service.
...
On the right-hand side, there are the requested interfaces. Among these there are the news, the water irrigation system and the humidity sensors.
...
The main components of the Web Server are:
\begin{itemize}
...
\subsubsection{Farmers location} In order to locate a farm the user has two options:
    \begin{itemize}
        ...
        \item GPS coordinates: if the address hasn't been inserted by the user or, if the \textit{LocationModule} don't find the address, the user can manually specify the coordinates with the help of the map service.
    \end{itemize}
