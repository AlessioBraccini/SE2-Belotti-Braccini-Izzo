...
\begin{itemize}
    ...
    \end{itemize}
    \item Version 1.2: final update before the release
    \begin{itemize}
        \item[--] Update component diagrams: add SteeringInitiativeManager Component
        \item[--] Update sequence diagrams with the new component
        \item[--] Update diagrams in section 5.2.1 with the new component
        \item[--] Update requirement mapping
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    The ranking service is linked through an interface to the internal database, on the other side the water irrigation service and the humidity sensor service are considered external services and are managed by the \textit{SensorsDataManager}. Finally SteeringInitiativeService is handled by the \textit{SteeringInitiativeManager}.
    ...
    The daily plan service and the ranking service are linked to the internal database, instead the answer request service communicates directly with the \textit{HelpRequestManager} component. Finally SteeringInitiativeService is handled by the \textit{SteeringInitiativeManager}.
    ...
    \item \textbf{SteeringInitiativeManager}: it manages the steering initiatives for both agronomists and policy makers. It communicates with the \textit{AgronomistManager} in order to let the user upload the report and with the \textit{PolicyMakerManager} in order to visualize it.
...
\begin{figure}[H]
    \includegraphics[scale=0.6, center]{assets/Test/generalTest.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.6]{assets/Test/farmLocationTest.png} 
...
After the first group implementation and testing, it's time for the implementation of the middle tier: \emph{RankingManager}, \emph{HelpRequestManager} (fig. \ref{fig: integration_RankingManager_HelpRequestManager}), \emph{ForumManager}(fig. \ref{fig: integration_ForumManager_LocationModule}) and \emph{SteeringInitiativeManager}.
\begin{figure}[H]
    \centering
    ...
    \qquad\qquad
    \subfloat[\centering SteeringInitiativeManager]{{\includegraphics[scale=0.7]{assets/Test/reportTest.png}}}%
    \caption{Integration of RankingManager, HelpRequestManager and SteeringInitiativeManager components}%
    \label{fig: integration_RankingManager_HelpRequestManager}%
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.55]{assets/Test/forumTest.png}%
