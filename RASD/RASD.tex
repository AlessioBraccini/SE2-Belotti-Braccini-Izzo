...
The system "as it is" does not provide specific hardware equipment in order to access to web site. To use all functionalities it's required to have an IT devices connected to the internet, eventually a user can decide to use GPS to insert (it's????) own location and only proper devices with GPS equipment can do this. 
...
First is an external APIs to access maps which will use the position acquired by the user settings to return an interactive map with a marker on that exact position.

Another API service used is the one of the DBMS system which will be adopted in order to query the database.

\subsubsection{Communication Interfaces}
Internet connection is mandatory in order to access to every formation that the system will display.
Users that want full functionalities have to have (BRUTTO) a GPS system on their device.
