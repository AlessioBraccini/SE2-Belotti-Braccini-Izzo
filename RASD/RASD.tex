...
\begin{enumerate}
    ...
    \item Ottavia has just passed her test to become a policy maker, so she decides to download and install DREAM in order to visualize the data concerning her job.
    \begin{itemize}
        \item She open the app and click on the "Sign Up" button
        \item She insert all its data in order to fill every mandatory field and press the "Confirm" button
        \item System verifies that the e-mail isn't already present in the database and if it's all ok send the conirmation back
        \item He check his mail account in order to verify that all it ends good
    \end{itemize}

    \item Apu is an expert policy maker, he want to know what are the latest data with reguard to soil humidity and wwater ittigation.
    \begin{itemize}
        \item He takes his phone and log in DREAM app 
        \item He press the "watch data" button
        \item The system retrive data from a database and show them to him
        \item He carefully look into the soil humidity data and compare them with the water irrigation data
        \item Staisfied he return to the home page
    \end{itemize} 
    ...
    \item Chandran is an old farmer, he recently decides to move out from his previous field in order to give to his son, Aakesh, a bigger field to cultivatea and also changing the crop type because the news report an imminent bug invasion that will destroy mais cultivation.
    \begin{itemize}
        \item He log in DREAM app
        \item He press the edit button in order to change his area and crop type
    ...
    \item Chakrika is a young farmer. She decided to insert the data reguarding last week production.
    \begin{itemize}
        \item She sat in front of her computer and log in DREAM
        \item System shows her the farmers home page
        \item She press the insert production data button
        \item She insert the data reguarding the last week production 
        \item She rewatch the insert data and press confirmation button 
        \item If it was all ok system send her to home page
    \end{itemize}
    ...
    \item Sonya is an agronomist that want to check who are the best farmers of her responsability area.
    \begin{enumerate}
...
\begin{longtable}{|c|p{0.69\textwidth}|}
    ...
    \stepcounter{RequirementCtr}
    R\arabic{RequirementCtr}    & The system must allow the users to change their location in any moment, except when they have connectivity issues.\\\hline%no, l'app senza internet non va
    \stepcounter{RequirementCtr}
    ...
    \stepcounter{RequirementCtr}
    R\arabic{RequirementCtr}    & The system must let Farmers insert their production information every day, even without connection.\\\hline  % noooo
    \stepcounter{RequirementCtr}
    R\arabic{RequirementCtr}    & If a Farmer has temporary connectivity issues while trying to upload production data, the system must keep them locally stored until connection is restablished. Once there is connection, data must be uploaded to the database.\\\hline    % nooo
    \stepcounter{RequirementCtr}
    ...
    \stepcounter{RequirementCtr}
    R\arabic{RequirementCtr}    & The system must notify Agronomists of unresolved requests of help from Farmers.\\\hline    % più di uno o solo uno?
    \stepcounter{RequirementCtr}
    ...
    \stepcounter{RequirementCtr}
    R\arabic{RequirementCtr}    & The system must allow the Agronomists to add a schedule for the day each day, even without connection.\\\hline  %nooooo
    \stepcounter{RequirementCtr}
...
\end{itemize}
    ...
    \paragraph{Use Cases Description}
        ...
        \begin{itemize}
            ...
            \begin{center}
                \begin{figure}[H]
                    \includegraphics[scale=0.55, center]{assets/Sequence diagrams/Policy Maker/View Specific Farmer Informations .png}
                    \caption{Policy Makers - View Specific Farmers Informations Sequence Diagram}
